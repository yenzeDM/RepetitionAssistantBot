...
async def change_days_handler(message: types.Message, state: FSMContext):
    if await show_all_added_material(message):
        await state.set_state(ChangedDaysBeforeRepetition.choose_change_type)
        await message.answer(Russian.CHANGE_DAYS_TYPE_OF_CHANGE, reply_markup=kb_for_change)
    else:
...
async def change_by_days_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(Russian.CHANGE_BY_DAYS)
